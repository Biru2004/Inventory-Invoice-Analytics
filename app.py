...
#         with col1:
#             quantity =st.number_input("Total Weight (kg)",
#                 min_value=1,
#                 value=1200
#             )
...
# -------------------------------------------------------
st.set_page_config(
    page_title="SigmaAI: Logistic Intelligence Portal",
    page_icon="📦",
    layout="wide"
)
...
This internal analytics portal leverages machine learning to  
- **Forecast freight costs accurately** (Based on Weight, Value & Distance)
- **Detect risky or abnormal vendor invoices**
- **Reduce financial leakage and manual workload**
""")
...
if selected_model == "Freight Cost Prediction":
    ...
    st.markdown("""
    **Objective:** Predict freight cost for a vendor invoice using **Weight**, **Invoice Value**, and **Route Distance**.
    """)

    with st.form("freight_form"):
        # Teen columns banaye hain taaki distance bhi fit ho jaye
        col1, col2, col3 = st.columns(3)

        with col1:
            weight = st.number_input("⚖️ Total Weight (kg)",
                min_value=1.0,
                value=1200.0,
                help="Shipment ka total wajan"
            )
        ...
        with col3:
            distance = st.number_input(
                ...
                min_value=1.0,
                value=500.0,
                help="Destination tak ki doori"
            )

        submit_freight = st.form_submit_button("🔮 Predict Freight Cost")

    if submit_freight:
        # Distance ko bhi input_data mein add kar diya hai
        input_data = {
            "Weight": [weight],
            "Dollars": [dollars],
            "Distance": [distance]
        ...
        st.info(f"Calculation based on {weight}kg being transported over {distance}km.")
